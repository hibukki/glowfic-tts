...
def write_casting_doc(storage: Storage) -> Path:
    ...
    spoilers stay out of the repo): centrality, art slot, opening lines, voice."""
    rows = casting_sheet(storage)
    icons = _icon_urls_by_voice_key(storage)
    openings: dict[str, str] = {}
    for chunk in storage.load_script().chunks:
        if chunk.voice_key not in openings:
            openings[chunk.voice_key] = " ".join(chunk.rich.plain().split())[:400]

    out = storage.base / "casting.md"
    prior_art = _existing_art_notes(out)  # preserve hand-written notes across regenerations

    lines = [f"# Casting — post {storage.post_id} (SPOILERS)", ""]
    lines.append("Generated by `glowfic-tts cast … --write`. Add art notes under each character.\n")
    for row in rows:
        ...
        lines += [
            ...
            f"- icon: {icons.get(row['character'], '—')}",
